                ...
                if encapsulation_type=="1":
                    ...
                    if encapsulation_type == "1" and eth_type == "0x00000800":
                        ...
                        if ipv4_protocol == "6" : 
                            ...
                            if tcp_flags_syn != "0":
                                outFile[7] += int(tcp_flags_syn)
                            if tcp_flags_fin != "0":
                                outFile[8] += int(tcp_flags_fin)                         
                        ...
                        if last_num_packets == packets_amount :
                            outFile[6] =+ 1 
                    # If the last ks is different from the ks, update the file
                    # Clean features and repeat until the last ks is the same
                    print(last_ks)
                    if last_ks == window-1 and packets_amount > 0 and packets_amount != last_num_packets:
                        while last_ks != ks:
